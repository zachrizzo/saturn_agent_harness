...
if [[ "$ENGINE" == "claude" && "$IS_RESUME" != "yes" ]]; then
  CLAUDE_SYSTEM_APPEND="$AGENT_PROMPT$SATURN_CLI_INSTRUCTIONS$SATURN_ORCHESTRATOR_INSTRUCTIONS"
  if [[ -n "$CLAUDE_SYSTEM_APPEND" ]]; then
    CLAUDE_APPEND_SYSTEM_PROMPT_FILE="$SESSION_DIR/system-append.txt"
    printf '%s\n' "$CLAUDE_SYSTEM_APPEND" > "$CLAUDE_APPEND_SYSTEM_PROMPT_FILE"
    export CLAUDE_APPEND_SYSTEM_PROMPT_FILE
  fi
fi
...
if [[ "$BUILD_TRANSCRIPT" == "first" ]]; then
  ...
  PROMPT_TO_SEND="$(saturn_build_replay_prompt "" "${PREV_CLI:-unknown}" "$CLI" "$TRANSCRIPT" "$PROMPT_USER_MESSAGE")"
fi
